...
// Lightweight FBM: 2 octaves max (mobile-friendly)
float fbm2(vec2 p) {
    float f = 0.0;
    float w = 0.5;
    for (int i = 0; i < 2; i++) {
...
// Cheaper analytic glow (avoids exp); tuned for smooth falloff
float glow(float dist, float radius) {
    float x = max(dist, 0.0) * radius;
    return 1.0 / (1.0 + x * x);
}
...
vec4 drawPanelShape(vec2 uv, vec2 center, float shapeType, vec3 baseColor, float energy, vec2 rot) {
    vec2 p = uv - center;
    ...
    float edgeGlow = glow(max(d, 0.0), 10.0); // Concentrated glow (cheaper)
...
vec3 applyLivingShapeMotion(vec3 color, vec2 pos, float time) {
    ...
    // Subtler & slower for cinematic stability
    float microMotion = noise(pos * 3.0 + time * 0.25) * 0.05;
    // Boost base scaling to 1.0 so color never gets dimmed below its original level, and oscillates upwards
    return color * (1.0 + 0.18 * breath + microMotion * u_motion);
}
...
vec3 applyScene(float sceneId, vec3 baseColor, vec2 uv, vec2 pos, float energy) {
    ...
    if (sceneId < 0.5) {
        ...
        // Scene 2: Neon Grid
        float n = fbm2(uv * 3.2 - vec2(0.0, u_time * 0.35));
...
void main() {
    ...
    // Precompute rotation once (avoid per-panel trig)
    ...
    for (int i = 0; i < 16; i++) {
        ...
        // [v4.2 Optimization] Aggressive early-out (no sqrt): squared radius check
        float maxR = 0.45 * sizeFactor;
        if (dot(p, p) > (maxR * maxR)) {
            continue;
        }
        ...
        vec4 panelRender = drawPanelShape(uv, pos, u_shapeType, activeColor, u_energy, rot);
